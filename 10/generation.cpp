...
//funzione che serve a calcolare la distanza tra due punti (serve per alcuni metodi)
...
double Generation::CalculatePathFitness(Path path, int N) {
    if (N != m_NStops) {
        cerr<<"Error in CalculatePathFitness: invalid path length"<<endl;
        return 0;
    } else {
        double sum=0;
        int PosStart;
        int PosStop;
        for (int i=1; i<m_NStops; i++) {
            PosStart = path.GetElement(i);
            PosStop = path.GetElement(i+1); 
            sum += distance(m_stops[PosStart-1],m_stops[PosStop-1]);
        }
        PosStart = path.GetElement(m_NStops);
        PosStop = path.GetElement(1);
        sum += distance(m_stops[PosStart-1],m_stops[PosStop-1]);
...
void Generation::CrossGen(Random& rnd,Path* new_paths,int N, double NewGen_Exponent) {

    int modified = 0;
    double OldFitness, NewFitness, acceptance;

    std::multimap <double,Path> new_map;

    //creo una nuova multimap con i path già presenti e quelli aggiunti all'interno

    for (int i=0; i<m_NChromosomes;i++) {   
        new_map.insert(pair<double,Path>(CalculateChromosomeFitness(i+1),m_chromosomes[i]));
    }

    for (int i=0; i<N;i++) {
        new_map.insert(pair<double,Path>(CalculatePathFitness(new_paths[i],m_NStops),new_paths[i]));

    }
    // utilizzando un procedimento analogo a quello usato per creare una nuova generazione, metto insieme i path
    ...
    int ChosenPlace;
    for (int i=0; i<m_NChromosomes; i++) {
        ChosenPlace = int((m_NChromosomes+N)*pow(rnd.Rannyu(),NewGen_Exponent))+1;
        m_chromosomes[i] = itr->second;
        itr++;
    }
