...
//Read initial configuration
  ...
  else 
  {
    ...
    for (int i=0; i<npart; ++i)
    {
      vx[i] = vx[i] - sumv[0];   //subtracting drift velocity
      vy[i] = vy[i] - sumv[1];
      vz[i] = vz[i] - sumv[2];
  ...
  for (int i=0; i<npart; ++i)
  {
    ...
    { //MD simulation. It's not a proper Verlet because it uses velocities,but works anyway
...
//Print initial values for measured properties
  ...
  cout<<endl<<"Potential tail corrections: " << 8.*M_PI*rho*(1./(9.*pow(rcut,9))-1./(3*pow(rcut,3))) << endl;
  cout<<"Virial tail corrections: "<< 32.*M_PI*rho*(1./(9.*pow(rcut,9))-1./(6.*pow(rcut,3))) << endl<<endl;
...
// distance i-j in pbc
      ...
      if(dr < rcut)
      {
        virij = 1.0/pow(dr,12) - 0.5/pow(dr,6);
        vir += virij;
  ...
  walker[ip] = 1./(3.*walker[iv])*48.*vir; //pressure
  walker[ig] = count_gr/(rho*npart*4/3*pi*pow(box/2,3));
...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Epot, Ekin, Etot, Temp, Press, g;
   const int wd=12;
    ...
    stima_pot = blk_av[iv]/blk_norm/(double)npart + 8.*M_PI*rho*(1./(9.*pow(rcut,9))-1./(3*pow(rcut,3))); //Potential energy
    ...
    stima_etot = blk_av[ie]/blk_norm/(double)npart + 8.*M_PI*rho*(1./(9.*pow(rcut,9))-1./(3*pow(rcut,3))); //Total energy
    ...
    stima_press = (rho*blk_av[it] + blk_av[ip] + 32.*M_PI*rho*(1./(9.*pow(rcut,9))-1./(6.*pow(rcut,3))))/blk_norm; //Pressure
    glob_av[ip] += stima_press;
    glob_av2[ip] += stima_press*stima_press;
    err_press = Error(glob_av[ip],glob_av2[ip],iblk);

    stima_g = blk_av[ig]/blk_norm;
    glob_av[ig] += stima_g;
    glob_av2[ig] +=stima_g*stima_g;
    err_gdir=Error(glob_av[ig],glob_av2[ig],iblk);
...
//g(r)
    g << setw(wd) << iblk <<  setw(wd) << stima_g << setw(wd) << glob_av[ig]/(double)iblk << setw(wd) << err_gdir << endl;
