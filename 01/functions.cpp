...
// funzioni utili per stampare vettori sullo standard output
...
// calcola il chi quadro in un vettore
double chi_squared (double* rand_numbers, int N, int L){
   ...
   for (int i=0; i<N; i++) {
      counter = 0;
      limmin = (double)i/(double)N; //limite inferiore del bin
      limmax = (double)(i+1)/(double)N; // limite superiore del bin
      for (int j=0; j<L; j++) {
...
// funzioni utili che stampano vettori su file di testo di nome filename 
...
// Inizializzazione per la variabile Random
